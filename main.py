...
LIVEKIT_URL = "wss://capstone-project-bqq0llqs.livekit.cloud"
...
def format_speed(num_bytes: int, elapsed_sec: float) -> str:
    if elapsed_sec <= 0:
        return "N/A"
    bytes_per_sec = num_bytes / elapsed_sec
    mbps = (bytes_per_sec * 8) / (1024 * 1024)
    return f"{format_bytes(int(bytes_per_sec))}/s ({mbps:.3f} Mbps)"
...
async def call_runpod_inference(manual_img_path, camera_frame_bytes):
    if not RUNPOD_INFERENCE_URL:
        return {"result": "WAIT", "reason": "camera connection waiting"}
    try:
        rel_path = manual_img_path.lstrip('/')
        full_manual_path = os.path.join(BASE_DIR, rel_path)
        if not os.path.exists(full_manual_path):
            return {"result": "ERROR", "reason": "manual image not found"}

        async with httpx.AsyncClient() as client:
            manual_size = os.path.getsize(full_manual_path)
            camera_size = len(camera_frame_bytes)
            request_payload_size = manual_size + camera_size
            request_start = time.perf_counter()
            request_wall_start = time.time()
            vlm_log_state["last_frame_send_start"] = request_start
            vlm_log_state["sent_frame_count"] += 1
            vlm_log_state["frame_send_times"].append(request_start)
            vlm_log_state["frame_send_times"] = [
                sent_at for sent_at in vlm_log_state["frame_send_times"]
                if request_start - sent_at < 1.0
            ]
            current_fps = len(vlm_log_state["frame_send_times"]) / 1.0
            with open(full_manual_path, "rb") as manual_file:
                files = {
                    "manual_image": ("manual.jpg", manual_file, "image/jpeg"),
                    "camera_image": ("camera.jpg", io.BytesIO(camera_frame_bytes), "image/jpeg")
                }
                data = {"prompt": "compare assembly step"}
                response = await client.post(RUNPOD_INFERENCE_URL, files=files, data=data, timeout=120.0)

            transfer_elapsed = time.perf_counter() - request_start
            response_wall_received = time.time()
            if response.status_code == 200:
                response_json = response.json()
                prediction = response_json.get("prediction", {"result": "UNKNOWN", "reason": "analysis error"})
                timing = response_json.get("timing", {})
                feedback = prediction.get("reason", "")
                server_received_at = timing.get("server_received_at")
                server_response_ready_at = timing.get("server_response_ready_at")
                if isinstance(server_received_at, (int, float)):
                    frame_send_ms = max(0.0, (server_received_at - request_wall_start) * 1000.0)
                    frame_send_log = f"{frame_send_ms:.3f} ms"
                else:
                    frame_send_log = "측정 불가 (GPU server_received_at 없음)"
                if isinstance(server_response_ready_at, (int, float)):
                    feedback_send_sec = max(0.0, response_wall_received - server_response_ready_at)
                    feedback_send_log = f"{feedback_send_sec:.3f} sec"
                else:
                    feedback_send_log = "측정 불가 (GPU server_response_ready_at 없음)"
                print(f"main.py -> GPU 서버로의 프레임 전송 시간: {frame_send_log}")
                print(f"피드백: {feedback}")
                print(f"GPU 서버 -> main.py로의 피드백 전송 시간: {feedback_send_log}")
                print(f"VLM 전체 왕복 시간: {transfer_elapsed:.3f} sec")
                return prediction
            return {"result": "ERROR", "reason": f"server error: {response.status_code}"}
    except Exception as e:
        return {"result": "ERROR", "reason": f"communication failed: {str(e)}"}
